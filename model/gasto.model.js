...
class GastoModel {
    static async readUltimo5GastoUsuarioModel(empresa, usuario,sucursal,semana) {
        try {
            ...
            var sql = "select FORMAT(CAST(sum(table1.gasto) AS DECIMAL(10, 2)), 2) as gasto from (select G.cantidad as gasto from gastos as G " +
                "where FK_CodigoUsuarioAdmin = '" + usuario + "' and week(DateTimeRegistroGasto) = "+semana+" and FK_CodeEmpresa = '" + empresa + "' and FK_CodeSucursal = "+sucursal+" order by G.DateTimeRegistroGasto desc limit 5) as table1;"
    ...
    static async readListUltimoGastoModel(sucursal)
    {
        try {
            var conn = await connDB().promise()
    ...
    static async readReporteGastoModel(sucursal)
    {
        try {
            var conn = await connDB().promise()
